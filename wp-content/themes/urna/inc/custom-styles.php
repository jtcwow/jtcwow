	...
	function urna_tbay_default_theme_primary_color() {
		...
		switch ($active_theme) {
			...
			case 'jewelry':
				...
				$theme_variable['font_family_second']	= true;
				break;
	...
	function urna_tbay_custom_styles() {
		...
		<?php if( $show_typography ) : ?>	
			...
			<?php if ( ($font_source == "2" && $main_google_font_face) || ($font_source == "3" && $main_custom_font_face) ): ?>
				<?php  echo trim($ouput['primary-font']); ?>
				{font-family: 
					<?php 
						switch ($font_source) {
							case '3':
			...
			<?php
				...
				$default_color = urna_tbay_default_theme_primary_color();

				if( !isset($default_color['font_family_second']) ) {
					$secondary_custom_font_face = $main_custom_font_face;
					$secondary_google_font_face = $main_google_font_face;
					$secondary_font 			= $main_font;
					$secondary_google_font_face = $main_google_font_face;
				}
			?>
			<?php if ( ($font_source == "2" && $secondary_google_font_face)  || ($font_source == "3" && $secondary_custom_font_face) ): ?>
					<?php  echo trim($ouput['secondary-font']); ?>
				{font-family:  
					<?php 
						switch ($font_source) {
							case '3':
		...
		<?php endif; ?>
			...
			<?php if ( $main_color != "" ) : ?>
                ...
                /**Fix theme color important**/
				.product-block .wcfmmp_sold_by_wrapper a:hover, 
				.product-block .wcfmmp_sold_by_wrapper a:focus,
		    ...
		    @media (max-width: 1199px)  and (min-width: 768px) {	       
				...
				/*background*/
				.topbar-device-mobile .top-cart a.wc-continue,.tbay-addon-newletter .input-group-btn input {
					background-color: <?php echo esc_html( $main_bg_color ) ?>;
				}
		   ...
		   <?php endif; ?>
            ...
            <?php if( !empty($bg_buy_now) ) : ?>
				...
				.mobile-btn-cart-click #tbay-click-buy-now {
					...
					border-color: <?php echo esc_html( $bg_buy_now ) ?>;
				}
				...
				.mobile-btn-cart-click #tbay-click-buy-now:focus {
					...
					border-color: <?php echo esc_html( urna_tbay_color_lightens_darkens( $bg_buy_now, -0.1) ); ?>;
				}  
			}
			<?php endif; ?>
