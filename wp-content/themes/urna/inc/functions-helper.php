...
if ( !function_exists('urna_tbay_share_js') ) {
	function urna_tbay_share_js() {
		 if ( urna_tbay_get_config('enable_code_share',false) && is_single() ) {
...
if ( !function_exists('urna_tbay_get_attachment_image_loaded') ) {
	function urna_tbay_get_attachment_image_loaded($attachment_id, $size = 'thumbnail', $attr = '', $echo = true)  {
		...
		if ( $image ) {
			...
			if( ! (bool) urna_tbay_get_global_config('enable_lazyloadimage',false) ) {
			...
			if ( empty( $attr['srcset'] ) ) {
				...
				if ( is_array( $image_meta ) && function_exists( 'urna_calculate_image_srcset' ) ) {
					$size_array = array( absint( $width ), absint( $height ) );
					$srcset     = urna_calculate_image_srcset( $size_array, $src, $image_meta, $attachment_id );
...
if ( !function_exists('urna_tbay_src_image_loaded') ) {
	function urna_tbay_src_image_loaded($src, $attr = '', $hwstring ='' , $echo = true)  {
...
if ( !function_exists('urna_tbay_get_variation_swatchs') ) {
    function urna_tbay_get_variation_swatchs() {
        ...
        if( !(defined('URNA_WOOCOMMERCE_ACTIVED') && URNA_WOOCOMMERCE_ACTIVED) ) return $swatchs;
...
if ( !function_exists('urna_tbay_get_custom_tab_layouts') ) {
  function urna_tbay_get_custom_tab_layouts() {
    ...
    if( !(defined('URNA_WOOCOMMERCE_ACTIVED') && URNA_WOOCOMMERCE_ACTIVED) ) return $tabs;
    $args = array(
      'posts_per_page'   => -1,
      'offset'           => 0,
      'orderby'          => 'date',
...
if ( ! function_exists( 'urna_tbay_checK_data_responsive' ) ) {
    function urna_tbay_checK_data_responsive($desktop, $desktopsmall, $tablet, $landscape_mobile, $mobile) {
    	$data_array = array();
...
if ( ! function_exists( 'urna_tbay_checK_data_responsive_carousel' ) ) {
    function urna_tbay_checK_data_responsive_carousel($columns, $desktop, $desktopsmall, $tablet, $landscape_mobile, $mobile) {
    	$data_responsive = urna_tbay_checK_data_responsive($desktop, $desktopsmall, $tablet, $landscape_mobile, $mobile);
...
if (!function_exists('urna_is_elementor_activated')) {
    function urna_is_elementor_activated() {
