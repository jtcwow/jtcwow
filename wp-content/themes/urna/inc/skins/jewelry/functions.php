...
if ( !function_exists('urna_fonts_url') ) {
	function urna_fonts_url() {
	    ...
	    */
	    ...
	    $PlayfairDisplay 		= _x( 'on', 'PlayfairDisplay font: on or off', 'urna' );
	    ...
	    if ( 'off' !== $Poppins || 'off' !== $PlayfairDisplay ) {
	        ...
	        if ( 'off' !== $PlayfairDisplay ) {
	            $font_families[] = 'Playfair Display:400,600';
	        }   
	...
	function urna_tbay_private_widgets_init() {
		...
		if( defined('URNA_CORE_ACTIVED') && URNA_CORE_ACTIVED ) {
			register_sidebar( array(
				'name'          => esc_html__( 'Banner Image', 'urna' ),
				'id'            => 'banner_image',
				'description'   => esc_html__( 'Add widgets here to appear in Banner Image', 'urna' ),
