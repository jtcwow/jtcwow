	...
	function urna_tbay_private_widgets_init() {
		...
		if( defined('URNA_CORE_ACTIVED') && URNA_CORE_ACTIVED ) {
			register_sidebar( array(
				'name'          => esc_html__( 'Top Info', 'urna' ),
				'id'            => 'top-info',
				'description'   => esc_html__( 'Add widgets here to appear in top info.', 'urna' ),
...
//Custom menu categories canvas
if ( ! function_exists( 'urna_custom_category_inside_title' ) ) {
	function urna_custom_category_inside_title( $output ) {
	    $output = '';
	    return $output;
	}
	add_filter( 'urna_category_inside_title', 'urna_custom_category_inside_title' );
}
...
if ( !function_exists ('urna_tbay_skin_custom_styles') ) {
	function urna_tbay_skin_custom_styles() {
		...
		/**Tablet**/
		$tablet_background = $ouput['tablet_background'];
		$tablet_black 	= $tablet_background. ', .tbay-search-form .button-group:before'; 

		/**Mobile**/
		$mobile_background 	= $ouput['mobile_background'];
		$mobile_black 		= $mobile_background. ', .tbay-search-form .button-group:before, .coupon .box:before,.tbay-show-cart-mobile .product-block div.add-cart a.added + a.added_to_cart'; 

		$main_skin_black = ', #shop-now.has-buy-now .tbay-buy-now.button:not(.disabled):hover, #shop-now.has-buy-now .tbay-buy-now.button:not(.disabled):focus, body table.compare-list .add-to-cart td a.button';
		...
		@media (max-width: 1199px)  and (min-width: 768px) {
			<?php if( isset($tablet_black) && !empty($tablet_black) ) : ?>
			<?php echo trim($tablet_black); ?> {
				color: #000;
			}
			<?php endif; ?>
		}
		...
		@media (max-width: 767px) {
			<?php if( isset($mobile_black) && !empty($mobile_black) ) : ?>
			<?php echo trim($mobile_black); ?> {
				color: #000;
			}
			<?php endif; ?>
		}
