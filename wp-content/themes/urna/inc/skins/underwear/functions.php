...
if ( !function_exists('urna_tbay_private_size_image_setup') ) {
	function urna_tbay_private_size_image_setup() {
		...
		set_post_thumbnail_size(471	, 301, true); // Unlimited height, soft crop
		update_option('thumbnail_size_w', 471);
		update_option('thumbnail_size_h', 301);						
...
if ( !function_exists('urna_fonts_url') ) {
	function urna_fonts_url() {
	    ...
	    */
		...
		$Mr_De_Haviland    = _x( 'on', 'Mr De Haviland font: on or off', 'urna' );
	    if ( 'off' !== $Poppins ) {
	        $font_families = array(); 
...
if ( ! function_exists( 'urna_skin_tbay_icon_canvas_menu' ) ) {
    function urna_skin_tbay_icon_canvas_menu() {
        $output = '<i class="linear-icon-menu"></i>';

        return $output;
    }
    add_filter( 'urna_get_icon_canvas_menu', 'urna_skin_tbay_icon_canvas_menu', 2 );
}
...
if ( !function_exists ('urna_tbay_skin_custom_styles') ) {
	function urna_tbay_skin_custom_styles() {
		...
		$main_skin_black = ', .product-block.v10 .group-buttons>div a:hover:before';
