...
if ( ! function_exists( 'urna_tbay_center_meta' ) ) {
	function urna_tbay_center_meta( $post ) { 
		...
		echo '<div class="entry-meta">';
			...
			"<span class='author'>". esc_html_e('/ By ', 'urna'); the_author_posts_link() .'</span>';
			echo '</div>';
		echo '</div>';
	}
}
...
if ( ! function_exists( 'urna_tbay_post_share_box' ) ) {
  function urna_tbay_post_share_box() {
      if ( urna_tbay_get_config('enable_code_share',false) && urna_tbay_get_config('show_blog_social_share') ) {
          ?>
	...
	function urna_tbay_post_nav() {
		...
		<nav class="navigation post-navigation">
			...
			<div class="nav-links clearfix">
				<?php
				if ( is_attachment() ) :
					previous_post_link( '%link','<div class="col-lg-6"><span class="meta-nav">'. esc_html__('Published In', 'urna').'</span></div>');
				else :
...
if (!function_exists('urna_get_display_header_builder') ) {
	function urna_get_display_header_builder() {
		...
		$posts = get_posts($args);  
		foreach ( $posts as $post ) {
			if( urna_is_elementor_activated() && Elementor\Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
				return Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post->ID, urna_get_elementor_css_print_method() );
			} else {
	...
	function urna_get_display_footer_builder($footer) {
		...
		foreach ( $posts as $post ) {
			if( urna_is_elementor_activated() && Elementor\Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
				...
				$ouput .= Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post->ID, urna_get_elementor_css_print_method() );
				$ouput .= '</footer>';
				...
				$ouput = '<footer id="tbay-footer" class="tbay-footer">';
					$ouput .= '<div class="footer"><div class="container">';
						$ouput .= do_shortcode( $post->post_content );
					$ouput .= '</div></div>';
				$ouput .= '</footer>';
...
if ( !function_exists('urna_tbay_wpml') ) {
	...
	add_action('urna_tbay_header_custom_language','urna_tbay_wpml', 10);
}
			...
			if( isset($blog_archive_layout) ) {
        		...
        		switch ( $blog_archive_layout ) {
       			...
       			if( $blog_archive_layout === 'left-main' && empty($configs['left']['sidebar'])) {
                    $configs['main'] = array( 'class' => '' );
                } else if( $blog_archive_layout === 'main-right' && empty($configs['right']['sidebar'])) {
			 		$configs['main'] = array( 'class' => '' );
                }
				...
				if( isset($blog_single_layout) ) {
					...
					if( $blog_single_layout === 'left-main' && empty($configs['left']['sidebar'])) {
	                    $configs['main'] = array( 'class' => 'col-xs-12 single-full' );
	                } else if( $blog_single_layout === 'main-right' && empty($configs['right']['sidebar'])) {
				 		$configs['main'] = array( 'class' => 'col-xs-12 single-full' );
	                }
...
if ( ! function_exists( 'urna_tbay_add_bg_close_canvas_menu' ) ) {
	function urna_tbay_add_bg_close_canvas_menu() {
		...
		if( !is_active_sidebar($sidebar_id) || urna_tbay_get_config( 'select-header-page', 'default' ) !== 'default' ) return;
		
		if( get_post_type() === 'tbay_footer'  ) return;
		?>
