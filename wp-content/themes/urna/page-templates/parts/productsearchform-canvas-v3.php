	...
	<div id="tbay-search-form-canvas-v3" class="tbay-search-form">
		...
		<div class="sidebar-canvas-search">
			...
			<div class="sidebar-content">
				...
				<form action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get" class="searchform urna-ajax-search" data-thumbnail="<?php echo esc_attr( $enable_image ); ?>" data-ajaxsearch="<?php echo esc_attr( $autocomplete_search ) ?>" data-price="<?php echo esc_attr( $enable_price ); ?>" data-minChars="<?php echo esc_attr( $minchars ) ?>" data-post-type="<?php echo esc_attr( $search_type ) ?>" data-count="<?php echo esc_attr( $number ); ?>">
				...
				<div class="input-group">
					<?php if ( urna_tbay_get_config('search_category') ): ?>
						...
						<div class="select-category input-group-addon">
							<?php if ( urna_tbay_get_config('search_type') == 'product' ) :
								$args = array(
				  	...
				  	<?php endif; ?>
				  		...
				  		<div class="input-results">
					  		<input data-style="right" type="text" placeholder="<?php echo esc_attr($search_placeholder); ?>" name="s" required oninvalid="this.setCustomValidity('<?php esc_html_e('Enter at least 2 characters', 'urna'); ?>')" oninput="setCustomValidity('')" class="tbay-search form-control input-sm"/>
