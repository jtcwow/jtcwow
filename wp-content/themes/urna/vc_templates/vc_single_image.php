...
switch ( $source ) {
	...
	case 'featured_image':
		...
		$img = array(
				'thumbnail' => urna_tbay_src_image_loaded($custom_src, array(
	...
	case 'external_link':
		...
		$img = array(
			'thumbnail' => urna_tbay_src_image_loaded($custom_src, array('class' => 'vc_single_image-img', 'alt' => ""), $hwstring , false),
		);
...
if ( ! $img ) {
	$img['thumbnail'] = urna_tbay_src_image_loaded($default_src, array('class' => 'vc_img-placeholder vc_single_image-img', 'alt' => ""), false);
}
...
$lazyload_class = ( (bool) urna_tbay_get_config('enable_lazyloadimage', true) ) ? 'tbay-image-loaded' : '';
 
$wrapperClass = 'vc_single_image-wrapper '. $lazyload_class .' ' . $style . ' ' . $border_color;
...
$class_to_filter = 'wpb_single_image tbay-addon wpb_content_element vc_align_' . $alignment . ' ' . $this->getCSSAnimation( $css_animation );
...
if ( 'yes' === $add_caption && '' !== $caption ) {
	$html .= '<figcaption class="vc_figure-caption">' . trim( $caption ) . '</figcaption>';
}

$output = '
	<div class="' . esc_attr( trim( $css_class ) ) . '">
		' . wpb_widget_title( array( 'title' => $title, 'extraclass' => 'wpb_single_image_heading tbay-addon-title' ) ) . '
