...
?>
        ...
        <p class="referral_code_panel form-row form-row-wide">
            ...
            <label for="referral_code"><?php echo apply_filters('wmc_reg_field_referral_field_name_change',__( 'Referral Code', 'wmc' )) . ( $wmc_required_referral_field == 'yes' ? ' <span class="required">*</span>' : '' ) ;?></label>

            <input type="text" <?php echo $readonly;?> placeholder="<?php echo __( 'Add referral code if you have', 'wmc' );?>"  class="input-text" <?php $wmc_required_referral_field == 'yes' ? 'required' : ''; ?> name="referral_code" id="referral_code" value="<?php echo isset($data['referral_code'])?$data['referral_code']:''; ?>" />           
