  ...
  public function column_default( $item, $column_name ) {
	...
	switch ( $column_name ) {
	  ...
	  case 'order_id':
		...
		case 'user_id':
            ...
            //print_r($order->get_billing());

            $name = $order_data['billing']['first_name'].' '.$order_data['billing']['last_name'];

			return  $name;

            //ucwords( get_user_meta( $item[$column_name], 'first_name', true ).' '.get_user_meta( $item[$column_name], 'last_name', true ) );
   ...
   function get_columns() {
	 ...
	 $columns = [
	   ...
	   'credits' => __( 'Total Credits from this Order', 'wmc' ),
   ...
   public function prepare_items() {
	...
	$post_per_page = get_option('posts_per_page');
	 ...
	 $total_items  = $search_result ? 0 : $obj_referal_program->record_count( 'credits', $all_record, $user_id, $order_id );
	 ...
	 $this->items = $search_result ? array() : $obj_referal_program->get_credits( $per_page, $current_page, $user_id, $order_id );
