...
if ( ! class_exists( 'WMC_Module' ) ) {
	...
	abstract class WMC_Module {
		...
		protected static function render_template( $default_template_path = false, $variables = array(), $require = 'once' ) {
			do_action( 'wmc_render_template_pre', $default_template_path, $variables );

			$template_path = locate_template( basename( $default_template_path ) );
			if ( ! $template_path ) {
				$template_path = dirname( __DIR__ ) . '/views/' . $default_template_path;
			}
			$template_path = apply_filters( 'wmc_template_path', $template_path );
			...
			if ( is_file( $template_path ) ) {
				...
				$template_content = apply_filters( 'wmc_template_content', ob_get_clean(), $default_template_path, $template_path, $variables );
			} else {
				$template_content = '';
			}

			do_action( 'wmc_render_template_post', $default_template_path, $variables, $template_path, $template_content );
		 ...
		 * @return bool
		 */
		abstract protected function is_valid( $valid = 'all' );
	} // end WMC_Module
}
