...
if ( ! class_exists( 'WooCommerce_Multilevel_Referal' ) ) {
	...
	class WooCommerce_Multilevel_Referal extends WMC_Module {
		...
		public static function load_resources() {
            ...
            $version = filemtime( WMC_DIR . 'js/woocommerce-multilevel-referral-admin.js' );
			wp_register_script(
			...
			$version = filemtime( WMC_DIR . 'js/woocommerce-multilevel-referral.js' );
			wp_register_script(
            ...
            $version = filemtime( WMC_DIR . 'css/admin.css' );
			wp_register_style(
				self::PREFIX . 'admin',
			...
			$version = filemtime( WMC_DIR . 'css/style.css' );
			wp_register_style(
				self::PREFIX . 'front',
...
<td>{expire_date}</td>
		...
		public function register_hook_callbacks() {
			...
			add_action( 'admin_init', array($this,'wmc_custom_activate_plugin' ));
		}

		function wmc_new_feature_notice(){
			require_once WMC_DIR . 'views/admin/new_features.php';
		}

		function wmc_custom_activate_plugin(){
		    if ( isset( $_GET['wmc_addons_active_nonce'] ) && wp_verify_nonce( $_GET['wmc_addons_active_nonce'], $_GET['plugin_slug'] ) ) {
		    	$result = activate_plugin( $_GET['plugin'] );
		        if ( is_wp_error( $result ) ) {
		            add_action('admin_notices', function() {
			        	echo '<div class="notice notice-error is-dismissible" ><p>'.__('Something went wrong! Try again later!', 'wmc').'</p></div>';
			        },11);
		        }
		    }
		    $wmc_version = get_option( 'wmc_version', 0 );
			if( $wmc_version != WMC_VER ){
				add_action( 'admin_notices', array($this,'wmc_new_feature_notice' ));
				update_option( 'wmc_version', WMC_VER );
			} 
		}
		...
		public function init() {
			try {
				if( is_admin() ){
                    ...
                    if(isset($_POST['action']) && $_POST['action']=='getReferralsDetails'){ 
                       $arrReturn=array('result'=>'failed','data'=>__('Invalid Request','wmc'));                       
                       if(isset($_POST['userId']) && is_numeric($_POST['userId']) && $_POST['userId']!==0){
                            $userId=sanitize_text_field($_POST['userId']);     
                            $obj_referal_users = new Referal_Users();
                            $referralDetails=$obj_referal_users->fnGetReferralsIdsByLevel($userId);
                            $arrReturn['result']='success';
                            $arrReturn['data']=$referralDetails;
                       }
                    ...
                    if(isset($_GET['getReferralsCount'])){
                        $obj_referal_users = new Referal_Users();
                        $userId=sanitize_text_field($_GET['getReferralsCount']);
                        $referralCount=$obj_referal_users->fnGetFollowersCount($userId);
                        if(is_numeric($referralCount)){
                            update_user_meta( $userId, 'total_referrals', $referralCount);
                            echo $referralCount;
                        }else{
                            echo '!';
                        }                        
        ...
        static function fnActiveParentMenu( $files ){
        ...
        static function fnActiveSubMenu( $submenu_file, $parent_file ){
        	global $submenu;
			if( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'wmc-banner' ){
				$submenu_file = '';	
			}
                    ...
                    if($filetype['type']=='image/jpeg' || $filetype['type']=='image/png'|| $filetype['type']=='image/gif'){
                        ...
                        if($bannerId!='' && $bannerId!=0){
                            ...
                            if(has_post_thumbnail($bannerId)){
                                $attachmentId=get_post_thumbnail_id( $bannerId );
                                $image_data = wp_get_attachment_image_src( $attachmentId, "full" );
		...
		public function in_active_user( $query ) {
			...
			if(isset( $_REQUEST['search_by_join_sdate'] )  && $_REQUEST['search_by_join_sdate'] != '' ){
				$where .= ' AND ru.join_date >= "' . $_REQUEST['search_by_join_sdate'] . ' 00:00:00" ';
			}
			if(isset( $_REQUEST['search_by_join_edate'] )  && $_REQUEST['search_by_join_edate'] != '' ){
				$where .= ' AND ru.join_date <= "' . $_REQUEST['search_by_join_edate'] . ' 00:00:00" ';
			}
			if( $where ){
        ...
        function registerCustomPostType( $title, $title_p, $name, $supports, $isShow=false ){
            $labels = array(
                'name' => _x('Add New Banner',''),//_x( "$title_p", 'post type general name' ),
