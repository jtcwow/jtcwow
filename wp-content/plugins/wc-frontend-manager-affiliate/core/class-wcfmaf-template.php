...
/**
 * @class 		WCFM Affiliate Template Class
 *
 * @version		1.0.0
 * @package		WC
 * @author 		WC Lovers
 */
class WCFMaf_Template {

    public $template_url;

    public function __construct() {
        $this->template_url = 'wcfm/affiliate/';
    }
    ...
    public function get_template_part($slug, $name = '') {
        ...
        if (!$template && $name && file_exists($WCFMaf->plugin_path . "views/{$slug}-{$name}.php"))
            $template = $WCFMaf->plugin_path . "views/{$slug}-{$name}.php";
