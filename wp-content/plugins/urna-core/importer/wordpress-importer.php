...
if ( class_exists( 'WP_Importer' ) ) {
    class WP_Import extends WP_Importer {
		...
		public function deleteCaches(){
			foreach( $this->caches as $cache ){
				if(  is_file( Tbay_IMPORT_SAMPLES_DIR.$cache.'.txt' ) ){
					@unlink(  Tbay_IMPORT_SAMPLES_DIR.$cache.'.txt' );
				}
			}
		}
		public function getCaches(){
			foreach( $this->caches as $cache ){
				if( file_exists( Tbay_IMPORT_SAMPLES_DIR.$cache.'.txt' ) ){
					$this->$cache = json_decode( file_get_contents( Tbay_IMPORT_SAMPLES_DIR.$cache.'.txt' ), true );
				}
			}
		}
    ...
    <?php
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_slug ) {
				_e( 'Menu item skipped due to missing menu slug', 'tbay-elementor' );
				echo '<br />';
				return;
			}
			...
			if ( ! $menu_id ) {
				printf( __( 'Menu item skipped due to invalid menu slug: %s', 'tbay-elementor' ), esc_html( $menu_slug ) );
				echo '<br />';
				return;
			} else {
